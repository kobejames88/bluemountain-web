...
! function(window, undefined) {
    ...
    var layer = {
        ...
        config: function(options, fn) {
            ...
            isLayui
                ?
                layui.addcss('modules/layer/' + options.extend) : layer.link('skin/' + options.extend);
        ...
        ready: function(callback) {
            var cssname = 'skinlayercss',
                ver = '302';
            isLayui ? layui.addcss('modules/layer/default/layer.css?v=' + layer.v + ver, callback, cssname) : layer.link('skin/default/layer.css?v=' + layer.v + ver, callback, cssname);
            return this;
        },
    ...
    Class.pt.auto = function(index) {
        ...
        function setHeight(elem) {
            ...
            elem.height(area[1] - 10 - titHeight - btnHeight - 2 * (parseFloat(elem.css('padding')) | 0)); // DIY
        }
        switch (config.type) {
            case 2:
                setHeight('iframe');
    ...
    Class.pt.move = function() {
        ...
        _DOC.on('mousemove', function(e) {
            ...
            if (config.resize && dict.resizeStart) {
                ...
                e.preventDefault();
                var min_width = config.area[0].replace('px', '');
                var min_height = config.area[1].replace('px', '');

                var m_width = dict.area[0] + X;
                var m_height = dict.area[1] + Y;
    ...
    Class.pt.callback = function() {
        ...
        layero.find('.layui-layer-max').on('click', function() {
            if ($(this).hasClass('layui-layer-maxmin')) {
                ...
                layero.show();
                layero.removeClass('fadeOutUp').addClass('layer-anim');
            } else {
    ...
    layer.style = function(index, options, limit) {
        ...
        if (!limit) {
            if (parseFloat(options.width) <= options.min_width) {
                options.width = options.min_width;
            }
            if (parseFloat(options.height) - titHeight - btnHeight <= options.min_height) {
                options.height = parseFloat(options.min_height);
            }
        }

        options.height += 2; // DIY border 2px
        if (options.top < 40) {
            // TODO 正确计算TOP值
            options.top = (win.height() - options.height) / 2
        }
        layero.css(options);
        ...
        if (type === ready.type[2]) {
            ...
            // DIY
            contElem.css({
                height: parseFloat(options.height) - 11 - titHeight - btnHeight - parseFloat(contElem.css('padding-top')) - parseFloat(contElem.css('padding-bottom'))
            })
        }
    };
    ...
    layer.min = function(index, options) {
        ...
        layero.removeClass('layer-anim').addClass('fadeOutUp');
