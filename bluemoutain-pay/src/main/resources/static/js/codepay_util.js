...
show_Qrcode = function (data) {
    ...
    if (user_data && user_data.qrcode_url) {
        data.qrcode = user_data.qrcode_url + '?money=' + data.money + '&qr_user='+o.qr_user+ '&type=' + data["type"] + '&tag=' + data.tag;
    }
...
function startNotiry(data) {
    ...
    // notifyServer = notify = io.connect(window.location.hostname=='127.0.0.1'?'http://192.168.31.201:52888':data.notiry_host);
    ...
    notify.on('notify', function (o) { //同步通知服务返回
        ...
        if (user_data && user_data.qrcode_url && o.money) {
            o.qrcode = user_data.qrcode_url + '?money=' + o.money + '&qr_user='+o.qr_user+'&type=' + o["type"] + '&tag=' + o.tag;
        }
    ...
    notify.on('sussecc', function (o) { //成功后跳转
        ...
        $("#show_qrcode").attr("src", 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==');
        $("#show_qrcode").attr("alt", '支付成功 跳转中');
        var wait_time=window.wait_time||1000;
        ...
        if (!isNaN(status) && status > 1)$("#msg h1").html('支付成功 <a href="'+openUrl+'">立即跳转</a>');
        setTimeout(function () {
